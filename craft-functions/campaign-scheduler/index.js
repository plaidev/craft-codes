...
/**
 * 現在日時が祝日であるかをチェックし、onHolidayに基づき接客の公開判定を行う。
 * 祝日でない場合は常にtrueを返す。
 * @param {{date: Date, holidays: string[], onHoliday:string}} param0 現在日時、祝日一覧、祝日時の挙動からなるオブジェクト
 * @returns {boolean} 接客の公開(true), 非公開(false) を返す
 */
function isEnable({ date, holidays, onHoliday = 'enable' }) {
  const currentDateString = getDateString(date);
  if (!holidays.includes(currentDateString)) {
    // 祝日でなければそもそもチェック不要
    return true;
  }
  return onHoliday === 'enable'; // ここで初めてonHolidayを見る
}
 ...
 * @param {Object} logger logger module
 * @returns {{ campaignId: string, onHoliday?: string }} campaignIdとonHolidayの組み合わせ
 */
function validateEventDescriptionAndConvertToCampaignSetting(description, logger) {
  ...
  const { campaign_id: campaignId, on_holiday: onHoliday } = parsedDescription;
  if (campaignId == null) {
    throw new Error('campaign_id is required');
  }
  if (onHoliday != null && !['enable', 'disable'].includes(onHoliday)) {
    throw new Error('on_holiday must be enabled or disabled');
  }

  return { campaignId, onHoliday };
}
 ...
 * @param {Object} logger logger module
 * @returns {Promise<{ campaignId: string, onHoliday?: string }[]>} 接客IDと祝日設定の配列
 */
async function getCampaignSchedule(calendar, logger) {
  ...
  const events = res.data.items.filter(
    item => parseISO(item.start.dateTime).getTime() <= new Date(datetimeNow).getTime()
  );
...
export default async function (data, { MODULES }) {
  ...
  await Promise.allSettled(
    TARGET_CAMPAIGNS.map(async campaignId => {
      ...
      const setting = enabledCampaignSettings.find(s => s.campaignId === campaignId);
      if (setting == null) {
      ...
      const enabled = isEnable({
        date: currentDatetimeJST,
        holidays,
        onHoliday: setting.onHoliday,
      });
      await action.postV2betaActionCampaignToggleenabled({
        id: campaignId,
        enabled,
      ...
      logger.log(
        `dateString: ${getDateString(
          currentDatetimeJST
        )}, time: ${currentDatetimeJST.toISOString()}, campaign: ${campaignId}, enabled: ${enabled}`
      );
    })
  );
}
